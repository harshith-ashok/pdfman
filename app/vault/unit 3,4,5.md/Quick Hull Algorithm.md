...
#### Introduction
The Quick Hull algorithm is used for finding the Convex Hull of a set of points in 2D space. It recursively divides and conquers by identifying key points that form the convex hull.

#### Algorithmic Process

1. **Initialization:** Choose a pivot point from the set.
2. **Partitioning:** Partition the remaining points into two subsets based on their relative positions to the pivot with respect to the line defined by the pivot and another chosen reference point (often the lowest y-coordinate).
3. **Recursive Hull Construction:**
   - Recursively find the Convex Hull for each subset.
   - Merge the results, ensuring that no duplicate points are included in the final hull.

#### [[Complexity Analysis]]

- **Time Complexity:** The time complexity of Quick Hull is generally $O(N \log N)$, where $N$ is the number of input points. This is because each recursive call processes half of the points and the merging step ensures that duplicates are not added.
- **Space Complexity:** $O(N)$ due to the storage required for the subsets and the recursion stack.
